...
int nvgpu_priv_cmdbuf_append_zeros(struct gk20a *g, struct priv_cmd_entry *e,
		u32 entries)
{
	int err = 0;
	u32 entry_size = 0U;

	if (!nvgpu_safe_add_u32_return(e->fill_off, entries, &entry_size)) {
		nvgpu_err(g, "Arithmetic Overflow");
		err = -EOVERFLOW;
		return err;
	}

	nvgpu_assert(entry_size <= e->size);
