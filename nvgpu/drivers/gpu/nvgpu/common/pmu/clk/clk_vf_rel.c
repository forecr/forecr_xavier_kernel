...
static s32 clkVfRelConstructVfPoint(struct gk20a *g,
				struct boardobjgrp          *pBoardObjGrp,
				struct clk_vf_point         *pVfPointTmp,
				u16   vfPointIdx)
{
	...
	// Sanity check.
	if ((pVfPointTmp == NULL) || (pBoardObjGrp == NULL)) {
		status = -EINVAL;
		goto clkVfRelConstructVfPoint_exit;
	}

	// Construct the CLK_VF_POINT.
	pVfPoint = nvgpu_construct_clk_vf_point(g, (void *)pVfPointTmp);
	if (NULL == pVfPoint) {
		status = -ENOMEM;
		nvgpu_err(g, "Could not allocate CLK_VF_POINT, idx = %d.", vfPointIdx);
		goto clkVfRelConstructVfPoint_exit;
	}

	//
	// Insert the constructed CLK_VF_POINT into the
	// CLK_VF_POINTS BOARDOBJGRP.
	//
	status = boardobjgrp_objinsert(pBoardObjGrp,
		(struct pmu_board_obj *)&pVfPoint->super, vfPointIdx);

	if (status != 0) {
		nvgpu_err(g,
			"Could not insert CLK_VF_POINT into CLK_VF_POINTS BOARDOBJGRP - vfPointIdx=%d, status=%d",
			vfPointIdx, status);
		goto clkVfRelConstructVfPoint_exit;
	}

clkVfRelConstructVfPoint_exit:
	return status;
}
		...
		struct clk_vf_rel *pVfRel, u8 clkDomainIdx)
{
	...
	struct mv_pmu_clk_clk_vf_rel_vf_entry_sec *pVfEntrySec;
	union {
		struct pmu_board_obj boardObj;
		struct clk_vf_point vfPoint;
		struct clk_vf_point_50 v50;
		struct clk_vf_point_50_freq v50Freq;
		struct clk_vf_point_50_volt v50Volt;
		struct clk_vf_point_50_volt_pri v50VoltPri;
		struct clk_vf_point_50_volt_sec v50VoltSec;
	} vfPointData;
	struct nvgpu_clk_pmupstate *pClk = g->pmu->clk_pmu;
	u8 stepCount = 0;
	u32 voltageMinuV = 0;
	u32 voltageStepSizeuV = 0;
	u8 i;
	u16 vfPointIdx;
	u8 curveIdx;
	u8 objType;
	...
	objType = pmu_board_obj_get_type(&pVfRel->super);
	// Init common params
	if (objType == NV2080_CTRL_CLK_CLK_VF_REL_TYPE_50_RATIO_VOLT) {
		voltageMinuV      = pClk->avfs_fllobjs->lut_min_voltage_uv;
		voltageStepSizeuV = pClk->avfs_fllobjs->lut_step_size_uv;
		stepCount         = pClk->avfs_fllobjs->lut_num_entries;
		if (stepCount > 0) {
			stepCount -= 1;
		}
	}

	// Generate primary VF curve.

	// Init
	memset((u8 *)&vfPointData, 0x0, sizeof(vfPointData));

	// Get VF equations.
	pVfEntryPri = &pVfRel->vf_entry_pri;

	if (pVfEntryPri->vfeIdx == NV2080_CTRL_PERF_VFE_EQU_INDEX_INVALID) {
		nvgpu_err(g,
			"VFE equation is not provided to generate primary vf curve. domain = %d, vf rel = %d.",
			clkDomainIdx,
			pVfRel->super.idx);
		status = -EINVAL;
		goto clkVfRelVfFlatten_SUPER_exit;
	}
	//
	// Determine the next free index in the CLK_VF_POINTS BOARDOBJGRP at which
	// to insert new CLK_VF_POINTs.
	//
	vfPointIdx = BOARDOBJGRP_NEXT_EMPTY_IDX(&pClk->clk_vf_pointobjs->super.super);

	//
	// Will start allocating CLK_VF_POINTs for this CLK_VF_REL at the current
	// vfPointIdx.
	//
	pVfEntryPri->vfPointIdxFirst = vfPointIdx;

	// VF flattening is dependent on the source type.
	switch (objType) {
	case NV2080_CTRL_CLK_CLK_VF_REL_TYPE_50_TABLE_FREQ:
	case NV2080_CTRL_CLK_CLK_VF_REL_TYPE_50_RATIO_FREQ:
	{
		// Generate frequency-based VF_POINT.
		vfPointData.boardObj.type =
			NV2080_CTRL_CLK_CLK_VF_POINT_TYPE_50_FREQ;

		// Specify the _FREQ class data.
		vfPointData.v50Freq.freqMHz = pVfRel->freq_max_mhz;

		// Construct and insert the CLK_VF_POINT.
		status = clkVfRelConstructVfPoint(g,
					&pClk->clk_vf_pointobjs->super.super,
					&vfPointData.vfPoint,
					vfPointIdx);
		if (status != 0) {
			nvgpu_err(g,
				"Could not construct and insert CLK_VF_POINT_FREQ for domain = %d, vf rel = %d, freqMhz = %d, status=%d",
				clkDomainIdx,
				pVfRel->super.idx,
				pVfRel->freq_max_mhz,
				status);
		...
		//
		// Update the last CLK_VF_POINT index to the most recently
		// allocated and inserted CLK_VF_POINT.
		//
		pVfEntryPri->vfPointIdxLast = (vfPointIdx);
		break;
	}
	case NV2080_CTRL_CLK_CLK_VF_REL_TYPE_50_RATIO_VOLT:
	{
		// Generate voltage-based VF_POINT.
		vfPointData.boardObj.type =
			NV2080_CTRL_CLK_CLK_VF_POINT_TYPE_50_VOLT_PRI;

		for (i = 0; i < stepCount; i++) {
			// Specify the _VOLT class data.
			vfPointData.v50Volt.source_voltage_uv = (voltageMinuV + i * voltageStepSizeuV);
			...
			if (status != 0) {
				nvgpu_err(g,
					"Could not construct and insert CLK_VF_POINT_VOLT for domain = %d, vf rel = %d, sourceVoltageuV = %d, status=%d",
					clkDomainIdx,
					pVfRel->super.idx,
					(voltageMinuV + i * voltageStepSizeuV),
					status);
				goto clkVfRelVfFlatten_SUPER_exit;
			}
	...
	{
		nvgpu_err(g,
			"Invalid VF relationship type = %d.",
			objType);
		status = -EINVAL;;
		goto clkVfRelVfFlatten_SUPER_exit;
	}
	...
	// Short-circuit if there is no secondary VF curve.
	if (pClk->clk_vfrelobjs->vf_entry_count_sec == 0) {
		goto clkVfRelVfFlatten_SUPER_exit;
	}
	...
	// to insert new CLK_VF_POINTs.
	//
	vfPointIdx = BOARDOBJGRP_NEXT_EMPTY_IDX((&(pClk->clk_vf_pointobjs->sec.super.super)));

	// Flatten all secondary VF curves.
	for (curveIdx = 0; curveIdx < pClk->clk_vfrelobjs->vf_entry_count_sec; curveIdx++) {
		pVfEntrySec = &pVfRel->vf_entries_sec[curveIdx];

		//
		// If the VF_REL's secondary VF data points to an invalid VFE Equation,
		// there is no sec VF curve specified for this VF_REL. This is NOT an
		// error, skip it.
		//
		if (NV2080_CTRL_PERF_VFE_EQU_INDEX_INVALID == pVfEntrySec->vfeIdx) {
			continue;
		}
		...
		{
			nvgpu_err(g,
				"Secondary VF curves are not supported on frequency based VF points.");
			status = -EINVAL;
			goto clkVfRelVfFlatten_SUPER_exit;
		}
		case NV2080_CTRL_CLK_CLK_VF_REL_TYPE_50_RATIO_VOLT:
		{
			...
			// vfPointIdx.
			//
			pVfEntrySec->vfPointIdxFirst = (vfPointIdx);

			// Generate voltage-based VF_POINT.
			vfPointData.boardObj.type =
				NV2080_CTRL_CLK_CLK_VF_POINT_TYPE_50_VOLT_SEC;

			// Init DVCO Offset Code to INVALID.
			vfPointData.v50VoltSec.dvcoOffsetCodeOverride =
				NV2080_CTRL_CLK_CLK_VF_POINT_DVCO_OFFSET_CODE_INVALID;

			for (i = 0; i < stepCount; i++) {
				// Specify the _VOLT class data.
				vfPointData.v50Volt.source_voltage_uv =
					(voltageMinuV + i * voltageStepSizeuV);

				// Construct and insert the CLK_VF_POINT.
				status = clkVfRelConstructVfPoint(g,
							&(pClk->clk_vf_pointobjs->sec.super.super),
							&vfPointData.vfPoint,
							vfPointIdx);
				if (status != 0) {
					nvgpu_err(g,
				...
				// allocated and inserted CLK_VF_POINT.
				//
				pVfEntrySec->vfPointIdxLast = (vfPointIdx++);
			}
			break;
		}
		default:
		{
			nvgpu_err(g,
				"Invalid VF relationship type = %d.", objType);
			status = -EINVAL;
			goto clkVfRelVfFlatten_SUPER_exit;
		}
		...
		struct pmu_board_obj **ppBoardObj, u16 size, void *pArgs)
{
	...
	if (pArgs == NULL || ppBoardObj == NULL) {
		status = -EINVAL;
		nvgpu_err(g, "pArgs or ppBoardObj is NULL");
		goto done;
	}
...
static s32 clkVfRel_PmuDataInit_SUPER(struct gk20a *g,
			struct pmu_board_obj *pBoardObj,
			struct nv_pmu_boardobj *pPmuData)
{
	struct nv_pmu_clk_clk_vf_rel_boardobj_set *pVfRelSet = NULL;
	struct clk_vf_rel *pVfRel  = NULL;
	s32 status = 0;
	...
	if (status != 0) {
		nvgpu_err(g,
			"Error initializing BOARDOBJ PMU data: status=%d",
			status);
		goto clkVfRel_PmuDataInit_SUPER_exit;
	}
	pVfRel    = (struct clk_vf_rel *)pBoardObj;
	pVfRelSet = (struct nv_pmu_clk_clk_vf_rel_boardobj_set *)pPmuData;

	// Set VF REL class specific params.
	pVfRelSet->railIdx      = pVfRel->rail_idx;
	pVfRelSet->bOCOVEnabled = pVfRel->b_ocov_enabled;
	pVfRelSet->freqMaxMHz   = pVfRel->freq_max_mhz;
	pVfRelSet->voltDeltauV  = pVfRel->volt_delta_uv;
	pVfRelSet->freqDelta    = pVfRel->freq_delta;
	pVfRelSet->vfEntryPri   = pVfRel->vf_entry_pri;

	nvgpu_memcpy((u8 *)pVfRelSet->vfEntriesSec, (u8 *)pVfRel->vf_entries_sec,
				(sizeof(struct mv_pmu_clk_clk_vf_rel_vf_entry_sec)
					* NV2080_CTRL_CLK_CLK_VF_REL_VF_ENTRY_SEC_MAX));

clkVfRel_PmuDataInit_SUPER_exit:
	nvgpu_pmu_dbg(g, " done status %x", status);
	return status;
}

static s32 clkVfRel_PmuDataInit_50(struct gk20a *g,
			struct pmu_board_obj *pBoardObj,
			struct nv_pmu_boardobj *pPmuData)
{
	struct nv_pmu_clk_clk_vf_rel_50_boardobj_set *pVfRel50Set = NULL;
	struct clk_vf_rel_50 *pVfRel50  = NULL;
	s32 status = 0;

	nvgpu_pmu_dbg(g, " ");
	// Call _SUPER class.
	status = clkVfRel_PmuDataInit_SUPER(g, pBoardObj, pPmuData);
	if (status != 0) {
		nvgpu_err(g,
			"Error initializing BOARDOBJ PMU data: status=%d",
			status);
		goto clkVfRel_PmuDataInit_50_exit;
	}
	pVfRel50    = (struct clk_vf_rel_50 *)pBoardObj;
	pVfRel50Set = (struct nv_pmu_clk_clk_vf_rel_50_boardobj_set *)pPmuData;

	// Set VF REL class specific params.
	pVfRel50Set->safeVfCurveTupleIdx  = pVfRel50->safe_vf_curve_tuple_idx;
	pVfRel50Set->vfTupleIdxFirst      = pVfRel50->vf_tuple_idx_first;
	pVfRel50Set->vfTupleIdxLast       = pVfRel50->vf_tuple_idx_last;
	pVfRel50Set->pickedVfTupleIdx     = pVfRel50->picked_vf_tuple_idx;

clkVfRel_PmuDataInit_50_exit:
	nvgpu_pmu_dbg(g, " done status %x", status);
	return status;
}


static s32 clkVfRel_PmuDataInit_50_RATIO(struct gk20a *g,
			struct pmu_board_obj *pBoardObj,
			struct nv_pmu_boardobj *pPmuData)
{
	struct nv_pmu_clk_clk_vf_rel_50_ratio_boardobj_set *pVfRelRatio50Set =
			(struct nv_pmu_clk_clk_vf_rel_50_ratio_boardobj_set *)pPmuData;
	struct clk_vf_rel_50_ratio *pVfRelRatio50 = (struct clk_vf_rel_50_ratio *)pBoardObj;
	s32 status = 0;


	nvgpu_pmu_dbg(g, " ");
	// Call CLK_PROG super class.
	status = clkVfRel_PmuDataInit_50(g, pBoardObj, pPmuData);
	if (status != 0) {
		nvgpu_err(g,
			"Error initializing CLK_VF_REL_50 PMU data: status=%d",
			status);
		goto clkVfRel_PmuDataInit_50_RATIO_exit;
	}

	// Copy-out type-specific parameters.
	nvgpu_memcpy((u8 *)pVfRelRatio50Set->ratio.secondaryEntries,
				(u8 *)pVfRelRatio50->ratio.slaveEntries,
				(sizeof(struct nv_pmu_clk_clk_vf_rel_ratio_secondary_entry)
					* NV2080_CTRL_CLK_CLK_VF_REL_RATIO_SECONDARY_ENTRIES_MAX));

clkVfRel_PmuDataInit_50_RATIO_exit:
	nvgpu_pmu_dbg(g, " done status %x", status);
	return status;
}
		...
		struct pmu_board_obj **ppBoardObj, u16 size, void *pArgs)
{
	...
	pVfRelRatio50->super.super.super.pmudatainit = clkVfRel_PmuDataInit_50_RATIO;
...
static s32 clkVfRel_PmuDataInit_50_RATIO_VOLT(struct gk20a *g,
			struct pmu_board_obj *pBoardObj,
			struct nv_pmu_boardobj *pPmuData)
{
	struct nv_pmu_clk_clk_vf_rel_50_ratio_volt_boardobj_set *pVfRelRatioVolt50Set =
		(struct nv_pmu_clk_clk_vf_rel_50_ratio_volt_boardobj_set *)pPmuData;
	struct clk_vf_rel_50_ratio_volt  *pVfRelRatioVolt50 =
		(struct clk_vf_rel_50_ratio_volt *)pBoardObj;
	s32 status = 0;

	nvgpu_pmu_dbg(g, " ");
	// Call CLK_VF_REL super class.
	status = clkVfRel_PmuDataInit_50_RATIO(g, pBoardObj, pPmuData);
	if (status != 0) {
		nvgpu_err(g,
			"Error initializing CLK_VF_REL_RATIO PMU data: status=%d",
			status);
		goto clkVfRelIfaceModel10PmuDataInit_50_RATIO_VOLT_exit;
	}

	// Set type-specific parameters.
	pVfRelRatioVolt50Set->ratioVolt.vfSmoothDataGrp =
		pVfRelRatioVolt50->ratioVolt.vfSmoothDataGrp;

	// Set CLK_VF_REL_50_RATIO_VOLT class specific params.
	pVfRelRatioVolt50Set->vfGenMethod                 = pVfRelRatioVolt50->vfGenMethod;
	pVfRelRatioVolt50Set->devinitVfeIdx               = pVfRelRatioVolt50->devinitVfeIdx;
	pVfRelRatioVolt50Set->favDeratedSubCurveVFEEqnIdx =
			pVfRelRatioVolt50->favDeratedSubCurveVFEEqnIdx;

clkVfRelIfaceModel10PmuDataInit_50_RATIO_VOLT_exit:
	nvgpu_pmu_dbg(g, " done status %x", status);
	return status;
}
...
{
	struct pmu_board_obj *pObjTmp = (struct pmu_board_obj *)pArgs;
	struct clk_vf_rel_50_ratio_volt *pVfRelRatioVolt50Tmp =
			(struct clk_vf_rel_50_ratio_volt *)pArgs;
	struct clk_vf_rel_50_ratio_volt *pVfRelRatioVolt50;
	s32 status = 0;
	...
	// Set BOARDOBJ interfaces.
	pVfRelRatioVolt50->super.super.super.super.pmudatainit = clkVfRel_PmuDataInit_50_RATIO_VOLT;
			...
			struct nvgpu_clk_vf_rels *pvfrelobjs)
{
	...
	nvgpu_memcpy((u8 *)&header, vfrel_table_ptr, sizeof(header));
	...
	//
	// Save off the secondary and VF entry counts.  These will be used
	// to allocate and iterate over arrays within CLK_VF_REL objects.
	//
	...
	for (i = 0; i < header.entry_count; i++) {
		...
		if (status != 0)
			break;
		//
		// Zero out the clkDomainData structure to remove whatever
		// changes were made from previous iteration.
		//
...
static s32 clk_vf_rel_pmudatainit(struct gk20a *g,
		struct boardobjgrp *pboardobjgrp,
		struct nv_pmu_boardobjgrp_super *pboardobjgrppmu)
{
	struct nv_pmu_clk_clk_vf_rel_boardobjgrp_set_header *pVfRelsSet =
	(struct nv_pmu_clk_clk_vf_rel_boardobjgrp_set_header *)pboardobjgrppmu;
	struct nvgpu_clk_vf_rels *pVfRels = (struct nvgpu_clk_vf_rels *)pboardobjgrp;
	s32 status = 0;
	...
	if (status != 0) {
		nvgpu_err(g, "Error updating pmu boardobjgrp for Clk_vf_rel. status - %d",
				status);
	}

	// Set CLK_VF_RELS-specific data
	pVfRelsSet->secondary_entry_count = pVfRels->slave_entry_count;
	pVfRelsSet->vf_entry_count_sec     = pVfRels->vf_entry_count_sec;
...
static s32 clk_vf_rel_pmudata_instget(struct gk20a *g,
		struct nv_pmu_boardobjgrp *pmuboardobjgrp,
		struct nv_pmu_boardobj **pmu_obj,
		u16 idx)
{
	struct nv_pmu_clk_clk_vf_rel_boardobj_grp_set  *pgrp_set =
		(struct nv_pmu_clk_clk_vf_rel_boardobj_grp_set *)
		(void *)pmuboardobjgrp;
...
s32 clk_vf_rel_sw_setup(struct gk20a *g)
{
	...
	if (pboardobjgrp->bconstructed) {
		nvgpu_pmu_dbg(g, "clk_vf_rel boardobjgrp already constructed, skipping reinit");
		return 0;
	}

	nvgpu_pmu_dbg(g, "Constructing clk_vf_rel boardobjgrp for first time");
	...
	pboardobjgrp->pmudatainit = clk_vf_rel_pmudatainit;
	pboardobjgrp->pmudatainstget  = clk_vf_rel_pmudata_instget;
