...
static int vgpu_css_attach(struct gk20a *g,
				...
				struct gk20a_cs_snapshot_client *cs_client)
{
	...
	ret = vgpu_css_attach(g, cs_client);
	if (ret) {
		return ret;
	}

	ret = vgpu_css_init_snapshot_buffer(g);
	return ret;
}
