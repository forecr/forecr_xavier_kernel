	...
	do {
		...
		if (msg.hdr.ctrl_flags == 0) {
			...
			if (nvgpu_is_enabled(g, NVGPU_GSP_SCHEDULER_VM)) {
				if (msg.hdr.unit_id == NV_GSP_UNIT_VM_SCHED) {
					status = gsp_vm_sched_event_handler(g, &msg);
					if (status != 0) {
						return status;
					}
				} else {
					nvgpu_err(g, "gsp vm sched event is not valid to unit");
					return -EINVAL;
				}
			}
		}
