...
#ifdef CONFIG_NVGPU_MULTIMEDIA_TRACE
		err = nvgpu_multimedia_alloc_trace_buf_mem(g, &g->nvdec->trace_buf_mem[inst_id]);
		if (err != 0) {
			nvgpu_err(g, "nvdec trace buffer alloc failed:%x", err);
		}
#endif
	}
...
s32 nvgpu_nvdec_sw_deinit(struct gk20a *g)
{
	...
	nvgpu_log(g, gpu_dbg_mme, " ");
#ifdef CONFIG_NVGPU_MULTIMEDIA_TRACE
	for (u32 inst_id = 0; inst_id < NVDEC_INST_MAX; inst_id++) {
		nvgpu_multimedia_free_trace_buf_mem(g, &g->nvdec->trace_buf_mem[inst_id]);
	}
#endif
