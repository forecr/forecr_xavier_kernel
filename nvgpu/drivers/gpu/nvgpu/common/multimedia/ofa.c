...
#ifdef CONFIG_NVGPU_MULTIMEDIA_TRACE
		err = nvgpu_multimedia_alloc_trace_buf_mem(g, &g->ofa->trace_buf_mem[inst_id]);
		if (err != 0) {
			nvgpu_err(g, "ofa trace buffer alloc failed:%x", err);
		}
#endif
	}
...
s32 nvgpu_ofa_sw_deinit(struct gk20a *g)
{
	...
	nvgpu_log(g, gpu_dbg_mme, " ");
#ifdef CONFIG_NVGPU_MULTIMEDIA_TRACE
	for (u32 inst_id = 0; inst_id < OFA_INST_MAX; inst_id++) {
		nvgpu_multimedia_free_trace_buf_mem(g, &g->ofa->trace_buf_mem[inst_id]);
	}
#endif
