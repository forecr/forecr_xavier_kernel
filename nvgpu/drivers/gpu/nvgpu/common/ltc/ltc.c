...
int nvgpu_init_ltc_support(struct gk20a *g)
{
	...
	if (g->ops.ltc.intr.configure != NULL) {
		if (nvgpu_gin_is_present(g)) {
			nvgpu_gin_stall_unit_config(g, NVGPU_GIN_INTR_UNIT_LTC,
					NVGPU_LTC_LEGACY_VECTOR_OFFSET,
					NVGPU_GIN_INTR_ENABLE);
			nvgpu_gin_stall_unit_config(g, NVGPU_GIN_INTR_UNIT_LTC,
					NVGPU_LTC_UE_VECTOR_OFFSET,
					NVGPU_GIN_INTR_ENABLE);
			nvgpu_gin_stall_unit_config(g, NVGPU_GIN_INTR_UNIT_LTC,
					NVGPU_LTC_CE_VECTOR_OFFSET,
					NVGPU_GIN_INTR_ENABLE);
		} else {
	...
	for (ltc = 0; ltc < ltc_count; ltc++) {
		for (lts = 0; lts < slices_per_ltc; lts++) {
			...
			(void)snprintf(stats[ltc][lts].name,
					sizeof(stats[ltc][lts].name), "%s", "ltc");
			(void)nvgpu_strnadd_u32(ltc_str, ltc,
							sizeof(ltc_str), 10U);
			(void)snprintf(stats[ltc][lts].name + strlen(stats[ltc][lts].name),
