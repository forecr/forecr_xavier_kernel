...
struct gops_nvenc {
	...
	/**
	 * @brief Reset and boot NVENC engine.
	 *
	 * @param g [in]		Pointer to GPU driver struct.
	 * @param dev [in]		NVENC engine's device struct.
	 *
	...
	/**
	 * @brief Sets stall and non-stall intr ctrl vectors for NVENC.
	 *
	 * @param g [in]                Pointer to GPU driver struct.
	 * @param inst_id [in]          NVENC engine's instance id
	 ...
	 *
	 * This function gets the interrupt ctrl message for NVENC as requested.
	 */
	 ...
	 */
	void (*interface_enable)(struct gk20a *g, u32 inst_id);
	 ...
	 * actions.
	 */
	void (*nvenc_isr)(struct gk20a *g, u32 inst_id);
	...
	/**
	 * @brief Dump nvenc status and debug registers.
	 *
	 * @param g [in]		Pointer to GPU driver struct.
	 * @param inst_id [in]		NVENC engine's instance id
	 ...
	 */
	void (*get_nvenc_fw)(struct gk20a *g, struct nvgpu_firmware **desc_fw,
			struct nvgpu_firmware **image_fw,
			struct nvgpu_firmware **pkc_sig);

	void (*set_safety_intr)(struct gk20a *g, u32 inst_id, bool enable);
};
