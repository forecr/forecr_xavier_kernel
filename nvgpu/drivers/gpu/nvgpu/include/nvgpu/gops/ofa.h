...
struct gops_ofa {
	...
	/**
	 * @brief Reset and boot OFA engine.
	 *
	 * @param g [in]		Pointer to GPU driver struct.
	 * @param dev [in]		OFA engine's device struct.
	 *
	...
	/**
	 * @brief Sets stall and non-stall intr ctrl vectors for OFA.
	 *
	 * @param g [in]				Pointer to GPU driver struct.
	 * @param inst_id [in]			OFA engine's instance id
	 ...
	 *
	 * This function gets the interrupt ctrl message for OFA as requested.
	 */
	void (*get_intr_ctrl_msg)(struct gk20a *g, u32 inst_id, bool enable,
			u32 *intr_ctrl, u32 *intr_notify_ctrl);

	/**
	 * @brief Enable error correction interrupts for OFA.
	 *
	 * @param g [in]               Pointer to GPU driver struct.
	 * @param inst_id [in]         OFA engine's instance id
	 * @param enable [in]          Parameter to enable/disable interrupt.
	 *
	 * This function ecc interrupts for OFA as requested.
	 */
	void (*set_ecc_intr)(struct gk20a *g, u32 inst_id, bool enable);
	 ...
	 * actions.
	 */
	void (*ofa_isr)(struct gk20a *g, u32 inst_id);
	...
	/**
	 * @brief Dump ofa status and debug registers.
	 *
	 * @param g [in]		Pointer to GPU driver struct.
	 * @param inst_id [in]		OFA engine's instance id
	...
	/**
	 * @brief Get OFA firmware names and load firmware.
	 *
	 * @param g [in]		Pointer to GPU driver struct.
	 ...
	 */
	void (*get_ofa_fw)(struct gk20a *g, struct nvgpu_firmware **desc_fw,
