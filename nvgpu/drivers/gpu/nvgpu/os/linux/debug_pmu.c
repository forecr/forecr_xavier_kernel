...
static int elpg_ms_stat_show(struct seq_file *s, void *data)
{
	struct gk20a *g = s->private;
	struct pmu_pg_stats_data pg_stat_data = { 0 };
	u64 total_ingating, total_ungating, residency, divisor, dividend;
	...
	/* We compute the residency on a scale of 1000 */
	dividend = nvgpu_safe_mult_u64(total_ingating, 1000UL);
	...
	seq_printf(s,
			"Time in ELPG_MS: %llu us\n"
			"Time out of ELPG_MS: %llu us\n"
			"ELPG_MS residency ratio: %llu\n"
			"ELPG_MS Entry Count: %u\n"
			"ELPG_MS Avg Entry latency %u us\n"
			"ELPG_MS Avg Exit latency %u us\n",
			total_ingating, total_ungating,
			residency, pg_stat_data.gating_cnt,
			pg_stat_data.avg_entry_latency_us,
			pg_stat_data.avg_exit_latency_us);
...
static int elpg_ms_stat_open(struct inode *inode, struct file *file)
{
	return single_open(file, elpg_ms_stat_show, inode->i_private);
}

static const struct file_operations elpg_ms_stat_fops = {
	.open		= elpg_ms_stat_open,
	.read		= seq_read,
	.llseek		= seq_lseek,
	.release	= single_release,
};
...
static int elpg_ms_transitions_show(struct seq_file *s, void *data)
{
	...
	total_gating_cnt = g->elpg_ms_gating_cnt + pg_stat_data.gating_cnt;
...
static int gpc_rg_stat_show(struct seq_file *s, void *data)
{
	struct gk20a *g = s->private;
	struct pmu_pg_stats_data pg_stat_data = { 0 };
	u64 total_ingating, total_ungating, residency, divisor, dividend;
	...
	total_ingating = g->gpc_rg_ingating_time_us +
			(u64)pg_stat_data.ingating_time;
	total_ungating = g->gpc_rg_ungating_time_us +
			(u64)pg_stat_data.ungating_time;
	divisor = total_ingating + total_ungating;
	...
	seq_printf(s,
			"Time in GPC_RG: %llu us\n"
			"Time out of GPC_RG: %llu us\n"
			"GPC_RG residency ratio: %llu\n"
			"GPC_RG Entry Count: %u\n"
			"GPC_RG Avg Entry latency %u us\n"
			"GPC_RG Avg Exit latency %u us\n",
			total_ingating, total_ungating,
			residency, pg_stat_data.gating_cnt,
			pg_stat_data.avg_entry_latency_us,
			pg_stat_data.avg_exit_latency_us);
...
static int gpc_rg_stat_open(struct inode *inode, struct file *file)
{
	return single_open(file, gpc_rg_stat_show, inode->i_private);
}

static const struct file_operations gpc_rg_stat_fops = {
	.open		= gpc_rg_stat_open,
	.read		= seq_read,
	.llseek		= seq_lseek,
	.release	= single_release,
};

static int gpc_rg_transitions_show(struct seq_file *s, void *data)
{
	struct gk20a *g = s->private;
	struct pmu_pg_stats_data pg_stat_data = { 0 };
	u32 total_gating_cnt;
	...
	total_gating_cnt = g->gpc_rg_gating_cnt + pg_stat_data.gating_cnt;
...
static int gpc_rg_transitions_open(struct inode *inode, struct file *file)
{
	return single_open(file, gpc_rg_transitions_show, inode->i_private);
}

static const struct file_operations gpc_rg_transitions_fops = {
	.open		= gpc_rg_transitions_open,
	.read		= seq_read,
	.llseek		= seq_lseek,
	.release	= single_release,
};

static int nvdec_elpg_stat_show(struct seq_file *s, void *data)
{
	struct gk20a *g = s->private;
	struct pmu_pg_stats_data pg_stat_data = { 0 };
	u64 total_ingating, total_ungating, residency, divisor, dividend;
	...
	total_ingating = g->nvdec_pg_ingating_time_us +
			(u64)pg_stat_data.ingating_time;
	total_ungating = g->nvdec_pg_ungating_time_us +
			(u64)pg_stat_data.ungating_time;
	divisor = total_ingating + total_ungating;
	...
	seq_printf(s,
			"Time in NVDEC_ELPG: %llu us\n"
			"Time out of NVDEC_ELPG: %llu us\n"
			"NVDEC_ELPG residency ratio: %llu\n"
			"NVDEC_ELPG Entry Count: %u\n"
			"NVDEC_ELPG Avg Entry latency %u us\n"
			"NVDEC_ELPG Avg Exit latency %u us\n",
			total_ingating, total_ungating,
			residency, pg_stat_data.gating_cnt,
			pg_stat_data.avg_entry_latency_us,
			pg_stat_data.avg_exit_latency_us);
...
static int nvdec_elpg_stat_open(struct inode *inode, struct file *file)
{
	return single_open(file, nvdec_elpg_stat_show, inode->i_private);
}

static const struct file_operations nvdec_elpg_stat_fops = {
	.open		= nvdec_elpg_stat_open,
	.read		= seq_read,
	.llseek		= seq_lseek,
	.release	= single_release,
};

static int nvdec_elpg_transitions_show(struct seq_file *s, void *data)
{
	struct gk20a *g = s->private;
	struct pmu_pg_stats_data pg_stat_data = { 0 };
	u32 total_gating_cnt;
	...
	total_gating_cnt = g->nvdec_pg_gating_cnt + pg_stat_data.gating_cnt;
...
static int nvdec_elpg_transitions_open(struct inode *inode, struct file *file)
{
	return single_open(file, nvdec_elpg_transitions_show, inode->i_private);
}

static const struct file_operations nvdec_elpg_transitions_fops = {
	.open		= nvdec_elpg_transitions_open,
	.read		= seq_read,
	.llseek		= seq_lseek,
	.release	= single_release,
};

static int nvenc_elpg_stat_show(struct seq_file *s, void *data)
{
	struct gk20a *g = s->private;
	struct pmu_pg_stats_data pg_stat_data = { 0 };
	u64 total_ingating, total_ungating, residency, divisor, dividend;
	...
	total_ingating = g->nvenc_pg_ingating_time_us +
			(u64)pg_stat_data.ingating_time;
	total_ungating = g->nvenc_pg_ungating_time_us +
			(u64)pg_stat_data.ungating_time;
	divisor = total_ingating + total_ungating;
	...
	seq_printf(s,
			"Time in NVENC_ELPG: %llu us\n"
			"Time out of NVENC_ELPG: %llu us\n"
			"NVENC_ELPG residency ratio: %llu\n"
			"NVENC_ELPG Entry Count: %u\n"
			"NVENC_ELPG Avg Entry latency %u us\n"
			"NVENC_ELPG Avg Exit latency %u us\n",
			total_ingating, total_ungating,
			residency, pg_stat_data.gating_cnt,
			pg_stat_data.avg_entry_latency_us,
			pg_stat_data.avg_exit_latency_us);
...
static int nvenc_elpg_stat_open(struct inode *inode, struct file *file)
{
	return single_open(file, nvenc_elpg_stat_show, inode->i_private);
}

static const struct file_operations nvenc_elpg_stat_fops = {
	.open		= nvenc_elpg_stat_open,
	.read		= seq_read,
	.llseek		= seq_lseek,
	.release	= single_release,
};

static int nvenc_elpg_transitions_show(struct seq_file *s, void *data)
{
	struct gk20a *g = s->private;
	struct pmu_pg_stats_data pg_stat_data = { 0 };
	u32 total_gating_cnt;
	...
	total_gating_cnt = g->nvenc_pg_gating_cnt + pg_stat_data.gating_cnt;
...
static int nvenc_elpg_transitions_open(struct inode *inode, struct file *file)
{
	return single_open(file, nvenc_elpg_transitions_show, inode->i_private);
}

static const struct file_operations nvenc_elpg_transitions_fops = {
	.open		= nvenc_elpg_transitions_open,
	.read		= seq_read,
	.llseek		= seq_lseek,
	.release	= single_release,
};

static int ofa_elpg_stat_show(struct seq_file *s, void *data)
{
	struct gk20a *g = s->private;
	struct pmu_pg_stats_data pg_stat_data = { 0 };
	u64 total_ingating, total_ungating, residency, divisor, dividend;
	...
	total_ingating = g->ofa_pg_ingating_time_us +
			(u64)pg_stat_data.ingating_time;
	total_ungating = g->ofa_pg_ungating_time_us +
			(u64)pg_stat_data.ungating_time;
	divisor = total_ingating + total_ungating;
	...
	seq_printf(s,
			"Time in OFA_ELPG: %llu us\n"
			"Time out of OFA_ELPG: %llu us\n"
			"OFA_ELPG residency ratio: %llu\n"
			"OFA_ELPG Entry Count: %u\n"
			"OFA_ELPG Avg Entry latency %u us\n"
			"OFA_ELPG Avg Exit latency %u us\n",
			total_ingating, total_ungating,
			residency, pg_stat_data.gating_cnt,
			pg_stat_data.avg_entry_latency_us,
			pg_stat_data.avg_exit_latency_us);
...
static int ofa_elpg_stat_open(struct inode *inode, struct file *file)
{
	return single_open(file, ofa_elpg_stat_show, inode->i_private);
}

static const struct file_operations ofa_elpg_stat_fops = {
	.open		= ofa_elpg_stat_open,
	.read		= seq_read,
	.llseek		= seq_lseek,
	.release	= single_release,
};

static int ofa_elpg_transitions_show(struct seq_file *s, void *data)
{
	struct gk20a *g = s->private;
	struct pmu_pg_stats_data pg_stat_data = { 0 };
	u32 total_gating_cnt;
	...
	total_gating_cnt = g->ofa_pg_gating_cnt + pg_stat_data.gating_cnt;
...
static int ofa_elpg_transitions_open(struct inode *inode, struct file *file)
{
	return single_open(file, ofa_elpg_transitions_show, inode->i_private);
}

static const struct file_operations ofa_elpg_transitions_fops = {
	.open		= ofa_elpg_transitions_open,
	.read		= seq_read,
	.llseek		= seq_lseek,
	.release	= single_release,
};
...
int gk20a_pmu_debugfs_init(struct gk20a *g)
{
	...
	d = debugfs_create_file(
		"elpg_ms_residency", S_IRUGO|S_IWUSR, l->debugfs, g,
						&elpg_ms_stat_fops);
	if (!d)
		goto err_out;
	...
	d = debugfs_create_file(
		"gpc_rg_residency", S_IRUGO|S_IWUSR, l->debugfs, g,
						&gpc_rg_stat_fops);
	if (!d)
		goto err_out;

	d = debugfs_create_file(
		"gpc_rg_transitions", S_IRUGO, l->debugfs, g,
						&gpc_rg_transitions_fops);
	if (!d)
		goto err_out;

	d = debugfs_create_file(
		"nvdec_elpg_residency", S_IRUGO|S_IWUSR, l->debugfs, g,
						&nvdec_elpg_stat_fops);
	if (!d)
		goto err_out;

	d = debugfs_create_file(
		"nvdec_elpg_transitions", S_IRUGO, l->debugfs, g,
						&nvdec_elpg_transitions_fops);
	if (!d)
		goto err_out;

	d = debugfs_create_file(
		"nvenc_elpg_residency", S_IRUGO|S_IWUSR, l->debugfs, g,
						&nvenc_elpg_stat_fops);
	if (!d)
		goto err_out;

	d = debugfs_create_file(
		"nvenc_elpg_transitions", S_IRUGO, l->debugfs, g,
						&nvenc_elpg_transitions_fops);
	if (!d)
		goto err_out;

	d = debugfs_create_file(
		"ofa_elpg_residency", S_IRUGO|S_IWUSR, l->debugfs, g,
						&ofa_elpg_stat_fops);
	if (!d)
		goto err_out;

	d = debugfs_create_file(
		"ofa_elpg_transitions", S_IRUGO, l->debugfs, g,
						&ofa_elpg_transitions_fops);
	if (!d)
		goto err_out;
