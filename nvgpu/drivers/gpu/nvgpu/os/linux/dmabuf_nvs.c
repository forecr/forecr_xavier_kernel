...
static int nvgpu_nvs_buf_mmap(struct dma_buf *dmabuf, struct vm_area_struct *vma)
{
	...
	if (!nvgpu_safe_add_u32_return(linux_buf->mapped_ref, 1, &linux_buf->mapped_ref)) {
		nvgpu_err(g, "linux_buf->mapped_ref overflow");
		zap_vma_entries(g, vma);
		err = -EOVERFLOW;
		goto fail;
	}
		...
		bool is_abrupt_flag)
{
	...
	nvgpu_nvs_buffer_free(sched_ctrl, buf, is_abrupt_flag);
	nvgpu_kfree(g, priv);
		...
		size_t bytes, u8 mask, bool read_only)
{
	...
	buf->priv = priv;
	buf->free_queue = nvgpu_nvs_destroy_buf_linux_locked;

	return 0;
fail:
	nvgpu_kfree(g, priv);
	nvgpu_nvs_buffer_free(sched_ctrl, buf, false);
