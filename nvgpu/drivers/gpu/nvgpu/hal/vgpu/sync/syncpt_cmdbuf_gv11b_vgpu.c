...
static int set_syncpt_ro_map_gpu_va_locked(struct vm_gk20a *vm)
{
	...
	if (vm->syncpt_ro_map_gpu_va[0] != 0U) {
		return 0;
	}

	vm->syncpt_ro_map_gpu_va[0] = nvgpu_vm_alloc_va(vm,
			g->syncpt_unit_size,
			GMMU_PAGE_SIZE_KERNEL, false, 0U);
	if (vm->syncpt_ro_map_gpu_va[0] == 0U) {
	...
	p->gpu_va = vm->syncpt_ro_map_gpu_va[0];
	...
	if (err) {
		nvgpu_err(g,
			...
			err);
		nvgpu_vm_free_va(vm, vm->syncpt_ro_map_gpu_va[0],
				 GMMU_PAGE_SIZE_KERNEL);
		vm->syncpt_ro_map_gpu_va[0] = 0;
		return err;
	}
	...
	u64 *base_gpuva, u32 *sync_size, u32 *num_syncpoints)
{
	...
	*base_gpuva = vm->syncpt_ro_map_gpu_va[0];
