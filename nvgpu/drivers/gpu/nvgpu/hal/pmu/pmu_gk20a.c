	...
	if (PMU_IS_COMMAND_QUEUE(queue_id)) {
		...
		if (!set) {
			*tail = pwr_pmu_msgq_tail_val_v(
				gk20a_readl(g, g->ops.pmu.pmu_get_msgq_tail()));
		} else {
			gk20a_writel(g,
				g->ops.pmu.pmu_get_msgq_tail(),
