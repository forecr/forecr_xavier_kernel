			...
			struct nvgpu_firmware **pkc_sig)
{
	...
	*desc_fw = nvgpu_request_firmware(g, GB20C_NVENC_RISCV_EB_UCODE_DESC,
					NVGPU_REQUEST_FIRMWARE_NO_WARN);

	if (nvgpu_acr_is_lsb_v3_supported(g)) {
		*pkc_sig = nvgpu_request_firmware(g,
				GB20C_NVENC_RISCV_EB_UCODE_LSB_PKC_SIG,
				NVGPU_REQUEST_FIRMWARE_NO_WARN);
	} else {
		*pkc_sig = nvgpu_request_firmware(g,
				GB20C_NVENC_RISCV_EB_UCODE_PKC_SIG,
				NVGPU_REQUEST_FIRMWARE_NO_WARN);
	}

	if (g->ops.pmu.is_debug_mode_enabled(g)) {
		*image_fw = nvgpu_request_firmware(g,
				GB20C_NVENC_RISCV_EB_UCODE_IMAGE,
				NVGPU_REQUEST_FIRMWARE_NO_WARN);
	} else {
		*image_fw = nvgpu_request_firmware(g,
				GB20C_NVENC_RISCV_EB_UCODE_PROD_IMAGE,
				NVGPU_REQUEST_FIRMWARE_NO_WARN);
	}
}
