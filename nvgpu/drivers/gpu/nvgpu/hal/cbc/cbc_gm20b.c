...
int gm20b_cbc_alloc_comptags(struct gk20a *g, struct nvgpu_cbc *cbc)
{
	...
	compbit_backing_size =
		...
		nvgpu_safe_mult_u32(nvgpu_safe_mult_u32(nvgpu_ltc_get_ltc_count(g),
							nvgpu_ltc_get_slices_per_ltc(g)),
				    nvgpu_ltc_get_cacheline_size(g));

	/* aligned to 2KB * ltc_count */
	compbit_backing_size = nvgpu_safe_add_u32(compbit_backing_size,
						  nvgpu_ltc_get_ltc_count(g) <<
						  ltc_ltcs_ltss_cbc_base_alignment_shift_v());

	/* must be a multiple of 64KB */
	compbit_backing_size = NVGPU_ALIGN(compbit_backing_size,
					   U32(64) * U32(1024));

	tmp =  nvgpu_safe_mult_u32(nvgpu_safe_mult_u32(nvgpu_ltc_get_ltc_count(g),
						       nvgpu_ltc_get_slices_per_ltc(g)),
				   nvgpu_ltc_get_cacheline_size(g));
	if (tmp == 0U) {
		return -EINVAL;
		       ...
		       u32 min, u32 max)
{
	...
	while (true) {
		const u32 iter_max = min(nvgpu_safe_sub_u32(nvgpu_safe_add_u32(min, max_lines),
							    1U), max);
		...
		for (ltc = 0; ltc < nvgpu_ltc_get_ltc_count(g); ltc++) {
			for (slice = 0; slice < slices_per_ltc; slice++) {
				...
				ctrl1 = nvgpu_safe_add_u32(ltc_ltc0_lts0_cbc_ctrl1_r(),
							   nvgpu_safe_add_u32(nvgpu_safe_mult_u32(ltc, ltc_stride),
									      nvgpu_safe_mult_u32(slice, lts_stride)));
...
u32 gm20b_cbc_fix_config(struct gk20a *g, u32 base)
{
	...
	if (val == 2U) {
		return (nvgpu_safe_mult_u32(base, 2U));
	} else if (val != 1U) {
...
void gm20b_cbc_init(struct gk20a *g, struct nvgpu_cbc *cbc, bool is_resume)
{
	...
	if (compbit_base_post_multiply64 < compbit_store_iova) {
		compbit_base_post_divide =  nvgpu_safe_add_u32(compbit_base_post_divide, 1U);
	}
