...
u32 gb10b_get_litter_value(struct gk20a *g, int value)
{
	u32 ret = 0;
	...
	case GPU_LIT_DMA_COPY_CLASS:
		ret = BLACKWELL_DMA_COPY_A;
		break;
	case GPU_LIT_NVENC_CLASS:
		ret = NVCEB7_VIDEO_ENCODER;
		break;
	case GPU_LIT_OFA_CLASS:
		ret = NVCEFA_VIDEO_OFA;
		break;
	case GPU_LIT_NVDEC_CLASS:
		ret = NVCEB0_VIDEO_DECODER;
		break;
	case GPU_LIT_NVJPG_CLASS:
		ret = NVCED0_VIDEO_NVJPG;
		break;
	...
	case GPU_LIT_LRCC_STRIDE:
		...
		break;
	case GPU_LIT_CTC_CHI_BRIDGE_PRIV_STRIDE:
		ret = proj_ctc_chi_bridge_stride_v();
		break;
	case GPU_LIT_NUM_CTC_CHI_BRIDGE_NUM_LINKS:
		ret = proj_scal_litter_ctc_chi_bridge_num_links_v();
		break;
	case GPU_LIT_NUM_GPCARB_PRI_TARGETS_PER_GPC:
		ret = proj_scal_litter_num_gpcarb_pri_targets_per_gpc_v();
		break;
	case GPU_LIT_PRI_GNIC_PRI_STRIDE:
		ret = proj_scal_litter_pri_gnic_pri_stride_v();
		break;
	case GPU_LIT_PRI_STRIDE_ACROSS_GNICS:
		ret = proj_scal_litter_pri_stride_across_gnics_v();
		break;
	default:
