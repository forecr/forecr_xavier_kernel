...
static int gb10b_netlist_silicon_get_name(struct gk20a *g, int index, char *name)
{
	...
	case NETLIST_FINAL:
		...
		if (g->ops.pmu.is_debug_mode_enabled(g)) {
			(void) snprintf(name, MAX_NETLIST_NAME, "%s",
								GB10B_NETLIST_DBG_IMAGE_FW_NAME);
		} else {
			(void) snprintf(name, MAX_NETLIST_NAME, "%s",
								GB10B_NETLIST_PROD_IMAGE_FW_NAME);
		}
#else
		if (g->ops.pmu.is_debug_mode_enabled(g)) {
			if (nvgpu_is_hypervisor_mode(g) && (g->support_gpu_tools == 0)) {
				(void) snprintf(name, MAX_NETLIST_NAME, "%s",
						GB10B_PROD_TEST_NSR_NETLIST_DBG_IMAGE_FW_NAME);
			} else {
				(void) snprintf(name, MAX_NETLIST_NAME, "%s",
						GB10B_NETLIST_DBG_IMAGE_FW_NAME);
			}
			...
			if (nvgpu_is_hypervisor_mode(g) && (g->support_gpu_tools == 0)) {
				(void) snprintf(name, MAX_NETLIST_NAME, "%s",
						GB10B_PROD_TEST_NSR_NETLIST_PROD_IMAGE_FW_NAME);
			} else {
				(void) snprintf(name, MAX_NETLIST_NAME, "%s",
						GB10B_NETLIST_PROD_IMAGE_FW_NAME);
			}
	...
	case NETLIST_SLOT_A:
		if (g->ops.pmu.is_debug_mode_enabled(g)) {
			if (nvgpu_is_hypervisor_mode(g) && (g->support_gpu_tools == 0)) {
				(void) snprintf(name, MAX_NETLIST_NAME, "%s",
							NVGPU_PROD_TEST_NSR_NETLIST_DBG_IMAGE_A);
			} else {
				(void) snprintf(name, MAX_NETLIST_NAME, "%s",
							NVGPU_NETLIST_DBG_IMAGE_A);
			}
		} else {
			if (nvgpu_is_hypervisor_mode(g) && (g->support_gpu_tools == 0)) {
				(void) snprintf(name, MAX_NETLIST_NAME, "%s",
							NVGPU_PROD_TEST_NSR_NETLIST_PROD_IMAGE_A);
			} else {
				(void) snprintf(name, MAX_NETLIST_NAME, "%s",
							NVGPU_NETLIST_PROD_IMAGE_A);
			}
		}
		break;
#endif
	...
	case NETLIST_SLOT_B:
		if (g->ops.pmu.is_debug_mode_enabled(g)) {
			(void) snprintf(name, MAX_NETLIST_NAME, "%s", NVGPU_NETLIST_DBG_IMAGE_B);
		} else {
			(void) snprintf(name, MAX_NETLIST_NAME, "%s", NVGPU_NETLIST_PROD_IMAGE_B);
		}
		break;
#endif
	...
	case NETLIST_SLOT_C:
		if (g->ops.pmu.is_debug_mode_enabled(g)) {
			(void) snprintf(name, MAX_NETLIST_NAME, "%s", NVGPU_NETLIST_DBG_IMAGE_C);
		} else {
			(void) snprintf(name, MAX_NETLIST_NAME, "%s", NVGPU_NETLIST_PROD_IMAGE_C);
		}
		break;
#endif
	...
	case NETLIST_SLOT_D:
		if (g->ops.pmu.is_debug_mode_enabled(g)) {
			(void) snprintf(name, MAX_NETLIST_NAME, "%s", NVGPU_NETLIST_DBG_IMAGE_D);
		} else {
			(void) snprintf(name, MAX_NETLIST_NAME, "%s", NVGPU_NETLIST_PROD_IMAGE_D);
		}
		break;
#endif
#endif
...
static int gb10b_netlist_sim_get_name(struct gk20a *g, int index, char *name)
{
	...
	case NETLIST_FINAL:
		(void) snprintf(name, MAX_NETLIST_NAME, "%s", GB10B_NETLIST_IMAGE_FW_NAME);
		break;
#else
#ifdef CONFIG_NVGPU_NON_FUSA
...
void gb10b_fecs_pkc_get_name(struct gk20a *g, char *name)
{
	...
	if (nvgpu_is_hypervisor_mode(g) && (g->support_gpu_tools == 0)) {
		(void) snprintf(name, MAX_NETLIST_NAME, "%s",
					GB10B_PROD_TEST_NSR_FECS_RISCV_UCODE_ENCRYPT_PKC_SIG);
	} else {
		(void) snprintf(name, MAX_NETLIST_NAME, "%s",
					GB10B_FECS_RISCV_UCODE_ENCRYPT_PKC_SIG);
	}
#else
	(void)g;
	(void) snprintf(name, MAX_NETLIST_NAME, "%s", GB10B_FECS_RISCV_UCODE_ENCRYPT_PKC_SIG);
#endif
}
...
void gb10b_gpccs_pkc_get_name(struct gk20a *g, char *name)
{
	...
	if (nvgpu_is_hypervisor_mode(g) && (g->support_gpu_tools == 0)) {
		(void) snprintf(name, MAX_NETLIST_NAME, "%s",
					GB10B_PROD_TEST_NSR_GPCCS_RISCV_UCODE_ENCRYPT_PKC_SIG);
	} else {
		(void) snprintf(name, MAX_NETLIST_NAME, "%s",
					GB10B_GPCCS_RISCV_UCODE_ENCRYPT_PKC_SIG);
	}
#else
	(void)g;
	(void) snprintf(name, MAX_NETLIST_NAME, "%s", GB10B_GPCCS_RISCV_UCODE_ENCRYPT_PKC_SIG);
#endif
}
...
void gb10b_fecs_desc_get_name(struct gk20a *g, char *name)
{
	...
	if (nvgpu_is_hypervisor_mode(g) && (g->support_gpu_tools == 0)) {
		(void) snprintf(name, MAX_NETLIST_NAME, "%s",
							GB10B_PROD_TEST_NSR_FECS_RISCV_UCODE_DESC);
	} else {
		(void) snprintf(name, MAX_NETLIST_NAME, "%s", GB10B_FECS_RISCV_UCODE_DESC);
	}
#else
	(void)g;
	(void) snprintf(name, MAX_NETLIST_NAME, "%s", GB10B_FECS_RISCV_UCODE_DESC);
...
void gb10b_gpccs_desc_get_name(struct gk20a *g, char *name)
{
	...
	if (nvgpu_is_hypervisor_mode(g) && (g->support_gpu_tools == 0)) {
		(void) snprintf(name, MAX_NETLIST_NAME, "%s",
							GB10B_PROD_TEST_NSR_GPCCS_RISCV_UCODE_DESC);
	} else {
		(void) snprintf(name, MAX_NETLIST_NAME, "%s", GB10B_GPCCS_RISCV_UCODE_DESC);
	}
#else
	(void)g;
	(void) snprintf(name, MAX_NETLIST_NAME, "%s", GB10B_GPCCS_RISCV_UCODE_DESC);
