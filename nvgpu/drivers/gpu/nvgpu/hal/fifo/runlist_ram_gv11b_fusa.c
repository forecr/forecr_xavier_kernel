...
{
	return ((RL_MAX_TIMESLICE_TIMEOUT << RL_MAX_TIMESLICE_SCALE) / 1000U) * 1024U;
}
...
{
	struct gk20a *g = ch->g;
	u32 addr_lo, addr_hi, runlist_entry;
	u32 userd_mask, inst_mask;

	userd_mask = nvgpu_aperture_mask(g, ch->userd_mem,
	...
	inst_mask = nvgpu_aperture_mask(g, &ch->inst_block,
	...
	runlist_entry = ram_rl_entry_type_channel_v() |
		...
		ram_rl_entry_chan_userd_target_f(userd_mask) |
		ram_rl_entry_chan_inst_target_f(inst_mask);
			...
			u32 *runlist)
{
	...
	userd_mask = nvgpu_aperture_mask_raw(g, userd_aperture,
					ram_rl_entry_chan_userd_target_sys_mem_ncoh_v(),
					ram_rl_entry_chan_userd_target_sys_mem_coh_v(),
					ram_rl_entry_chan_userd_target_vid_mem_v());
	inst_mask = nvgpu_aperture_mask_raw(g, inst_block_aperture,
					ram_rl_entry_chan_inst_target_sys_mem_ncoh_v(),
					ram_rl_entry_chan_inst_target_sys_mem_coh_v(),
					ram_rl_entry_chan_inst_target_vid_mem_v());
	...
	addr_lo = u64_lo32(inst_block_addr) >>
			ram_rl_entry_chan_inst_ptr_align_shift_v();
	addr_hi = u64_hi32(inst_block_addr);
