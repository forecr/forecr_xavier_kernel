...
static const char *const gb10b_fault_type_descs[] = {
	[0x00U] = "invalid pde",
...
static const char *const gb10b_fault_client_type_descs[] = {
	"gpc",
	"hub",
};

static const char *const gb10b_hub_client_descs[] = {
	[0x00U] = "vip",
	[0x01U] = "ce0",
	[0x02U] = "ce1",
...
static const char *const gb10b_gpc_client_descs[] = {
	[0x00U] = "t1_0",
	[0x01U] = "t1_1",
	[0x02U] = "t1_2",
	[0x03U] = "t1_3",
...
void gb10b_mm_mmu_fault_parse_mmu_fault_info(struct mmu_fault_info *mmufault)
{
	...
	if (mmufault->fault_type >= ARRAY_SIZE(gb10b_fault_type_descs)) {
		nvgpu_do_assert();
		mmufault->fault_type_desc = mmufault_invalid_str;
	} else {
		mmufault->fault_type_desc =
			 gb10b_fault_type_descs[mmufault->fault_type];
	}

	if (mmufault->client_type >= ARRAY_SIZE(gb10b_fault_client_type_descs)) {
		nvgpu_do_assert();
		mmufault->client_type_desc = mmufault_invalid_str;
	} else {
		mmufault->client_type_desc =
			 gb10b_fault_client_type_descs[mmufault->client_type];
	}

	mmufault->client_id_desc = mmufault_invalid_str;
	if (mmufault->client_type == gmmu_fault_client_type_hub_v()) {
		if (mmufault->client_id < ARRAY_SIZE(gb10b_hub_client_descs)) {
			mmufault->client_id_desc =
				 gb10b_hub_client_descs[mmufault->client_id] ==
					NULL ? "TBD" :
				gb10b_hub_client_descs[mmufault->client_id];
		} else {
			nvgpu_do_assert();
		...
		if (mmufault->client_id < ARRAY_SIZE(gb10b_gpc_client_descs)) {
			mmufault->client_id_desc =
				 gb10b_gpc_client_descs[mmufault->client_id] ==
					NULL ? "TBD" :
				gb10b_gpc_client_descs[mmufault->client_id];
		} else {
			nvgpu_do_assert();
